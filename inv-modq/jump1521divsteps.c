...
void gf_polymul_768x768_2x2_x_2x2_onlyv (int *M, int *M1, int *M2); // M = M2 x M1 
...
void gf_polymul_768x768_2x2_x_2x2 (int *M, int *M1, int *M2){ //only v = g^-1 mod f
    ...
    for (i=768, X=M+1536, Y=B1521_1; i>0; i--) {	// r = x r2 u1 + s2 r1
     T = barrett_16x2i(__SADD16(*X,*(Y++)));
       *(X++) = T;
    }
    ...
    for (i=768, X=M+2304, Y=B1521_1; i>0; i--) {	// s = x r2 v1 + s2 s1
     T = barrett_16x2i(__SADD16(*X,*(Y++)));
       *(X++) = T;
    }
...
void gf_polymul_768x768_2x2_x_2x2_onlyv (int *M, int *M1, int *M2){ //only v = g^-1 mod f
    int i, T, *X, *Y;
/*
    gf_polymul_768x768(BB1521_1, M2, M1); // x * u2 * u1 
    gf_polymul_768x768(M, M2+384,M1+768); // v2 * r1
...
int jump1521divsteps(int minusdelta, int *M, int *f, int *g){
    ...
    gf_polymul_768x768_2x2_x_2x2_onlyv(M+768,M1+768,M2+768);
